...
#read the table with updates
data_new <- read.csv("database_fire_20200530.csv",sep=",",
                     header=T, dec=".", stringsAsFactors=F)

###
#Add records to the tab1/qry/form.quadrat_samples?
pgInsert(con, qry, data.obj= data_new)

#Other option
for(record in data_frm)
{
  write_sql <- paste("Insert into tab1 (visit_id, sample_nr,
                     species,species_code,agegroup,resprout_organ,
                     seedbank,resprouts_live, resprouts_died, resprouts_kill,
                     resprouts_reproductive,recruits_live,recruits_died, recruits_reproductive,comments) values (",data_frm[,"emp_id"],",",data_frm[,"emp_sal"],",",data_frm[,"emp_bonus"],",",data_frm[,"emp_desig_level"],")",sep="")
