...
#Query para fecha de los incendios:
...
plant.traits %>%
  ggplot(aes(x=resprout_organ,y=recruit.dens, fill=resprout_organ)) +
  geom_boxplot(outlier.shape=NA) + geom_jitter(width=0.1,alpha=0.2) +
  facet_wrap(~seedbank, scales="free")
...
#Create a new data frame subsetting plant without reprout organ
seeders <- subset(plant.traits, resprout_organ == "none" )
str(seeders)
#16 spp
data.frame(seeders$species1, seeders$resprout_organ)
write.csv(seeders, "seeders.csv")
...
seeders %>%
  ...
  ggplot(aes(x=species1, y= recruit.dens, fill = seedbank)) +
...
#Only 6 spp have live recruits

##########
#RESPROUTERS
#For spp. with resprout organ = rhizome
#Calculate % reprout alive (resprout alive/ live + died + kill)
table(my.table$resprout_organ)
#109 records with rhizoma. Lets try this firts

table(plant.traits$resprout_organ)
my.rhizoma <- subset(plant.traits, resprout_organ =="rhizoma")
str(my.rhizoma)
#29 obs. of  34 variables
str(my.rhizoma)
...
my.rhizoma %>%
  ...
  ggplot(aes(x=species1, y=surv.dens, fill = seedbank)) +
  geom_bar(stat="identity",  alpha=.6, width=.4) +
  coord_flip() +
...
#Resprouts survival density is higher for  7 spp.
my.rhizoma %>%
  mutate(name = fct_reorder(species1, sprout.surv)) %>%
  ggplot(aes(x=species1, y=sprout.surv, fill = seedbank)) +
  geom_bar(stat="identity", alpha=.6, width=.4) +
  coord_flip() + xlab("") +
  theme_bw()+
  labs(title = "Post-fire sprouter survival", subtitle = "100*(N1)/(N1+N7)")+
  theme(text = element_text(size=12),
        strip.text.x = element_text(size=30),
        strip.text.y = element_text(size=12))
